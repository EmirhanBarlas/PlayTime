...
public class Playtime implements TabExecutor {
    ...
    public void checkAndCreateReport() {
        // Günlerden Pazar ve saat 23:59 kontrolü
        LocalDateTime now = LocalDateTime.now();
        if (now.getDayOfWeek() == DayOfWeek.SUNDAY && now.getHour() == 23 && now.getMinute() == 59) {
            // Oyuncuların playtime bilgilerini al
            TopPlayers[] top10 = getTopTen();
            top10 = checkOnlinePlayers(top10);

            // Dosya ismini tarih ve saatle oluştur
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss");
            String formattedDateTime = now.format(formatter);
            String fileName = "plugins/PlaytimeReports/report_" + formattedDateTime + ".txt";

            // .txt dosyasına yaz
            try {
                File txtFile = new File(fileName);
                txtFile.getParentFile().mkdirs(); // Alt klasörü oluştur
                ...
                for (int i = 0; i < top10.length; i++) {
                    ...
                    writer.write("Position: " + (i + 1) + ", Player: " + top10[i].name + ", Playtime: "
